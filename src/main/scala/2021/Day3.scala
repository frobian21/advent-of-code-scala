...
...###....#.....#...#..#.##..#."""
  ...
  def countTrees(unsplit_input: String)(right: Int, down: Int): Int =
    ...
    (0 to (input.length -1) / down).map(i => input((i*down))((i*right) % input(0).length)).count(_ == '#')    
  ...
  def calculateAllSlopes(input: String): Long = getAllSlopesForTrees(input).map(_.toLong).product

  println(calculateAllSlopes(example_input))
}