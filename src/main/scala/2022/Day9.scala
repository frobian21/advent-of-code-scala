...
enum RopeMove(val unitVector: Coordinate):
  ...
  case Left(steps: Int) extends RopeMove(-1, 0)
  case Right(steps: Int) extends RopeMove(1, 0)
  case Up(steps: Int) extends RopeMove(0, 1)
  case Down(steps: Int) extends RopeMove(0, -1)
end RopeMove
...
object Day9 {
  def parseRopeMoves(input: String): Array[RopeMove] =
    input.split("\n").collect{
      case RopeMove.moveRegex("L", steps) => RopeMove.Left(steps.toInt.ensuring(_ > 0))
      case RopeMove.moveRegex("R", steps) => RopeMove.Right(steps.toInt.ensuring(_ > 0))
      case RopeMove.moveRegex("U", steps) => RopeMove.Up(steps.toInt.ensuring(_ > 0))
      case RopeMove.moveRegex("D", steps) => RopeMove.Down(steps.toInt.ensuring(_ > 0))
    }
  
  def moveRopeHead(coordinatesPassed: Array[Coordinate], move: RopeMove): Array[Coordinate] =
    coordinatesPassed ++ (1 to move.steps).map(i => coordinatesPassed.last + (move.unitVector * i)).toArray
